...
let questionsData = [];
let selectedCategories = JSON.parse(sessionStorage.getItem('categories')) || [];
...
fetch('data/questions.json')
  ...
  .then(data => {
    ...
    // فلترة الأسئلة بناءً على الفئات المختارة
    if (selectedCategories.length > 0) {
      questionsData = data.filter(q => selectedCategories.includes(q.categoryId));
    }
